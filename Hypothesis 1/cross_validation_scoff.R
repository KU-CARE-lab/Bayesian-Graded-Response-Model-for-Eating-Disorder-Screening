...
load("result_scoff_square.Rdata")
...
generated quantities {
   int ED_dx;
   ED_dx = bernoulli_logit_rng(b_o*other + b_o_1*theta*other
                                   + b_f*FM + b_m*M
...
for(i in 1:data$I){
  ...
  result <- extract(fit)
  prob[i] <- mean(result$ED_dx)
  thetas[i] <- mean(result$theta)
  print(i)
}

save(thetas,file="cross_validation_scoff_theta_2022.Rdata")
