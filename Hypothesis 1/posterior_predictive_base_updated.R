...
data$I <- dim(x)[1]
data$N <- 10    # SCOFF
data$Y <- x[,grep("base",names(x))[1:5]]
data$K <- 5
data$ED_dx <- x$ED_dx
data$Z <- x$ED_dx
...
load("test_excessive_exercise.Rdata")

post <- extract(result$fit)
...
save(result,file="post_pre_base.Rdata")
...
lower <- c(0,6,10,13,17)
upper <- c(5,9,12,16,32)
...
for(i in 1:5){
  for(j in 1:dim(result$response_total_est)[1]){
    pp[j] <- mean(ifelse(result$response_total_est[j,]>=lower[i] &
                           result$response_total_est[j,]<=upper[i],1,0))
  }
  pp.t <- mean(ifelse(x$base_10item>=lower[i] & x$base_10item<=upper[i],1,0))
...
true_freq <- rep(0,41)
est_freq <- array(dim=c(1000,41))
g <- ifelse(!is.na(data$base) & data$M==1,1,0)*(1:data$I)
g <- g[g>0]

for(k in 0:40){
  true_freq[k+1] <- sum(ifelse(data$base[g]==k,1,0))/length(g)
  for(j in 1:1000){
...
boxplot(est_freq,main="BASE score: Cisgender men",xlab="Sum score",ylab="Proportion",
        at=0:40,outline=F,names=0:40)
lines(0:40,true_freq,type="o",col='red')
...
true_freq <- rep(0,41)
est_freq <- array(dim=c(1000,41))
g <- ifelse(!is.na(data$base) & data$FM==1,1,0)*(1:data$I)
g <- g[g>0]
...
boxplot(est_freq,main="BASE score: Cisgender women",xlab="Sum score",ylab="Proportion",
        at=0:40,outline=F,names=0:40)
lines(0:40,true_freq,type="o",col='red')
...
true_freq <- rep(0,41)
est_freq <- array(dim=c(1000,41))
g <- ifelse(!is.na(data$base) & data$other==1,1,0)*(1:data$I)
g <- g[g>0]
...
boxplot(est_freq,main="BASE score: Non-cisgender",xlab="Sum score",ylab="Proportion",
        at=0:40,outline=F,names=0:40)
lines(0:40,true_freq,type="o",col='red')
